function [result] = isWin(resultWord)
lengthOfResultWord = strlength(resultWord); 
    ...
    while chance > 0
        ...
        fprintf("If you want to guess all the letter at once, please enter 'wholeWord' into the dialogue box\n");
        fprintf("If you change your mind, please enter 'cancle' in the box\n");
        userInput = input("\nWhat is your letter/word?:\n","s");
        

        %Check if the user input is valid or not
        %If input is not valid then prompt the user to enter again.
        ...
        if strcmpi(userInput, "wholeWord")
            ...
            userInput = strtrim(string(userInput{1,1}));  

            %If user don't want to enter the whole word anymore, then enter
            %'cancle' 
            if strcmpi(userInput, "cancle")
                continue; 
            end
            ...
            %entered or not       
            fprintf("The word that the user enters is: %s", userInput);
            ...
            if isGuessTheWholeWordCorrect(resultWord,userInput)
                ...
                guessingArray = [guessingArray, string(userInput)]; 
                chance = chance - 1; 
